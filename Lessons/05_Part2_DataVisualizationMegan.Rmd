...
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) #default is gray
print(O3plot)
...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +    #no grid lines
  theme_classic()
print(O3plot4)
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) +  #changes font size
  theme(axis.text = element_text(color = "black"), #changes axis text color 
...
theme_set(mytheme) #sets the theme to be the theme you defined (mytheme) and all of your plots will show up with this theme event without you specifying it in each code

O3plot6 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot6)  #this plot is generated using mytheme without having to specify it because you set the theme in the code above
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) +  #dashed horizontal line 
  geom_hline(yintercept = 100, lty = 2) +  #dashed horizontal line (lty makes dashed)
  geom_point(alpha = 0.5, size = 1.5) +  #alpha determines transparency
...
#re-copied from original rmd because axis labels weren't showing up--worked after copying this back over
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
...
# hjust = 1 means left align margin, text allow you to add text to the plot where you specify
#the %b is the 3 letter abbreviation for month
#the [3] makes a subscript
```
...
display.brewer.all(n = 9) #all the brewer color maps
##^these let you see the different color map shades
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) +  #alpha adds transparency
  labs(color="Depth", shape="Lake name") + # change your legend labels 
  scale_shape_manual(values = c(15, 17)) +   #changes the shapes of your two variables
...
#allows you to divide you plot area into as many subsections as you want to display multiple plots at one time. You specify the plots you would like displayed and the number of rows to have in the display area. Rel high lets you determine how much space each plot will take up in the display area. So the 1.25 is giving the NvsP2 more space than the second plot
