...
PeterPaul.chem.nutrients <- read.csv("/home/guest/EDA-Fall2022/Data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)
...
## Linear Regression
A linear regression is comprised of a continuous response variable (y), plus a combination of 1+ continuous response variables (x), (plus the error term). The deterministic portion of the equation describes the response variable as lying on a straight line, with an intercept (alpha) and a slope term. The equation is thus a typical algebraic expression: 
...
REMEMBER null hypothesis means there's no relationship betweeen the two variables, so slope and intercept are zero
...
## Simple Linear Regression (only 1 variable on right side of the equation)
...
#remember lm function stands for linear model.  You provide the lm function with the dataframe and your y variable (in this case, irradiance)and then the dataframe and your x value

# another way to format the lm() function (the preferred way)
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression)

#The first thing R reports is the residuals then your coefficients. Under Estimate, you will have one for intercept that provides the intercept of the equation, then your x variable will be listed with a value that indicates the slope of the regression line. 

#For depth and irradiance, the slope is -95.890, meaning, as depth increases, irradiance decreases (makes sense: as depth increases, there's less light)
...
So, we see there is a significant negative correlation between irradiance and depth (lower light levels at greater depths), and that this model explains about 31 % (from the r2 value) of the total variance in irradiance. Let's visualize this relationship and the model itself. 
...
1. **Residuals vs. Fitted.** The value predicted by the line of best fit is the fitted value (returns a scatter plot of deterministic features), and the residual is the distance of that actual value from the predicted value. By definition, there will be a balance of positive and negative residuals. Watch for drastic asymmetry from side to side or a marked departure from zero for the red line - these are signs of a poor model fit.

2. **Normal Q-Q.** Normal Quantile-Quantile plot. Compares plot to normal distribution. The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. 
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #dividing plot viewer into four grids (2x2) and mar is size of those plots
plot(irradiance.regression) #provide plot function with the linear model object. Ideally, red line is as flat as possible for residuals vs fitted. For normal Q-Q, values should be on dotted linen which would be normal distribution. Residuals vs. leverage helps to identify outliers. Values outside of the dashed lines are outliers and look for any drastic departures from the solid red line. These are indicators of "poor model fit". There's an outlier so you can remove it by changing the axis limit.
par(mfrow = c(1,1))
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) +
  geom_point() 
print(irradiancebydepth) #ylim doesn't remove values, just excludes them
```
...
```{r, fig.height = 3, fig.width = 4}
#to be able to take the log, you need to eliminate zeroes, that's what the !=0 is doing.
...
#after log transformation, depth explains ~73% of variation in irrandiance (r squared value)

par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(irradiance.regression2)
par(mfrow = c(1,1))
#residual vs fitted: looking for symmetry along red line
...
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                   tp_ug ~ depth + dissolvedOxygen) #subsetting the whole dataframe to only have observations from Paul Lake
...
corrplot(PeterPaulCorr, method = "ellipse") #creates a correlation plot matrix. Imagine each ellipse as encompassing a scatterplot. THe more narrow the ellipse, the thinner the scatter plot, so a stronger correlation.
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #you can tell R to only print the upper diagnoal, since the matrix is symmetric. Everything on the upper has a mirror image on the lower. Mixed is both value of the correlation and then shape, you tell R to do the upper as the shape (ellipse so the lower is the correlation coefficient)
```

## AIC to select variables

However, it is possible to over-parameterize a linear model. Adding additional explanatory variables takes away degrees of freedom, and if explanatory variables co-vary the interpretation can become overly complicated. Remember, an ideal statistical model balances simplicity and explanatory power--there's a fine line between enough variables to explain relationships and too many that it becomes complicated! To help with this tradeoff, we can use the **Akaike's Information Criterion (AIC)** to compute a stepwise regression that either adds explanatory variables from the bottom up or removes explanatory variables from a full set of suggested options. The smaller the AIC value, the better. 
...
#the ~ symbol is saying tp_ug "is a function of" depth, DO, temperature, tn_ug, and po4

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC) #step function
#when you remove po4, your AIC is 352, when you remove depth, AIC is 352.76, etc. Lower AIC is better. So removing variables that are lowering your AIC could make your model stronger. However, removing DO, for example, would increase the AIC worsening your linear model. It then shows how the AIC changes if you remove one variable at a time, starting with PO4, then depth. SO after looking at each stepwise removal, you can determine which linear regression model would be best considering that variables you selected.
