...
### Some important caveats ---LOOK FOR TERMS OF SERVICE. Type robots.txt at the end of the URL to see prohibitions on data scraping
...
```{r scrape.some.data}
#you get the path in quotes by using the selector gadget on the website in chrome and making sure all of the boxes you want are in green and everything you do not want is in red by clicking

the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% 
  html_text()    #pulls the text associated with that node
the_registrant
```
...
avg_withdrawals <- webpage %>% 
  ...
  html_text()
avg_withdrawals
#note that these are pulled as characters--always pulls as characters
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant,  #the !! is referring to the variables you scraped
...
#Construct the scraping web address, i.e. its URL; you can construct the website URL from it's various components 
...
#Set the element address variables (determined in the previous step--those things you pulled with the gadget)
...
scrape.it <- function(the_year, the_facility){
  ...
  #Pause for a moment - scraping etiquette--this slows the system down (by 1 second in this case, but you can do more than that) so when you are scraping bulk data it doesn't look like an attack on a website which could trigger a denial of service/seem unethical
...
#Use lapply to apply the scrape function--lapply is iterative 
...
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #map2 allows you to iterate through each item in each collection (the ids and the years)
