...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul)#drop levels will get rid of the lakes you didn't specify
summary(NTL.nutrient.data.PeterPaul$lakename)
...
#these are the same operations as above, but done via piping
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #
  mutate(month = month(sampledate)) %>% #
  select(lakeid:daynum, month, sampledate:comments) %>% # 
  drop_na(depth) #drops NAs in column depth)
...
#this is selecting all of the columns that you want to keep, then making a subset of that data
  
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, 
                                             lakename, year4, daynum, month, sampledate, depth:po4)
#this is selecting the columns you want and creating a new subset dataframe
...
#this is getting rid of NAs in the concentration 
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient)#for each nutrient, how many observations are there
...
# Spread nutrient data into separate columns (this is the outdated method)
#NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration)

# Spread nutrient data into separate columns using pivot_wider. This does the opposite of pivot longer
...
### rbind
#rbind is similar to column bind, it binds by row
...
However, there are 20 months in this dataset, so importing all these files individually would be tedious to code. Here is a more efficient way to import and combine all files by using the list.files function by specifying where all of the files are located 
