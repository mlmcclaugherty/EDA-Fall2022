...
#install.packages('formatR') #wraps text so comments don't run off of the page
...
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))   #%in% is basically picking lakes named Paul and Peter within lakename column
...
#It's a row, so you use filter


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
TuesdayLake1990.1999<-filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Row numbers are arbitrary, they change 
