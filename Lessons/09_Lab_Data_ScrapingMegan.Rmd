...
#Get the data, which starts on line 30. Readtable is for tab separated data (no comma between columns); skip 29 because you don't want anything before the 29th line which is where the data actually starts
...
#all you have to do now is update the site number part of the URL for the new gage and update the period 
EnoURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30.
Enogage_data <- read.table(EnoURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 

#Update the column headers
colnames(Enogage_data) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
Enogage_data <- Enogage_data %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(Enogage_data,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
#Rename columns to something more useful--using the rename...function it will automatically do it for you
neuse <- renameNWISColumns(neuse); 
colnames(neuse)
  
#Compute annual means--aggregating by year
neuse_by_year <- neuse %>% 
...
nc_pop <- 
  get_acs(geography = "county",    #could use blockgroup, tract, etc
          variables = "B01003_001", #code for total population for counties
          state = "NC",
          geometry = TRUE,
          key = "d4ad7874c996c7a83fec0a81466309ddc07d8e22")  
#geometrt gives you a spatial dataframe so you can map it 
...
the_states <- the_website %>% 
  html_nodes('.basic-table a , th:nth-child(1)') %>% 
  html_text()
...
#3 Construct a dataframe from the values--gotta fix NAs from changing to numeric
df_Electric <- data.frame("State" = (the_states),
...
scrape.it <- function(the_year){
  #Get the proper url--if the year is 2021, use this URL, if it's not use the one following the paste0 function
...
df_Electric <- data.frame("State" = (the_states),
                          ...
                          "Net_Generation" = as.numeric(net_generation),
                          "Total_Retail" = as.numeric(total_retail))
  #Fetch the website ^^^^ all of this was copied from the above step into this scrape.it mega function
...
dfmanyyears <- map(seq(2017,2019), scrape.it) %>% bind_rows()
