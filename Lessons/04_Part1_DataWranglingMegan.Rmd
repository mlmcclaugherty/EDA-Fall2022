...
#stringAsFactors allows summary function to read characters
#2 Run some data frame functions to check out the dataset and make sure it was imported correctly
...
`==`compares your observations to a specific value/condition
`!=`checks if values are different than a specified value
`<`
`<=`
`>`
`>=`
`&` allows you to check if mulitple conditions are true
`|` checks if one or the other conditions are true

```{r}
class(NTL.phys.data$lakeid)
class(NTL.phys.data$depth)

# matrix filtering--you can use this type (rather than dplyr) because dataframes work as matrices
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #selects all Paul or Peter Lake
...
#the ! excludes those rows
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#%in% is a cleaner way of filtering; it still only selects those two types of observations


# Choose a range of conditions of a numeric or integer variable. These are several ways to do the same thing, returning data from June thru Oct1 by using the day number column in the original dataset
...
#year4 is an integer class. You can use filter since it works for integers
NTL.phys.data.Year1999 <- filter(NTL.phys.data, year4==1999)


# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.TuesdayLake1990to1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 >=1990 & year4 <=1999)
...
#you can use the colon : symbol to select sample date through temperature 
...
#this allows you to add a new column with temperature in fahrenheit, the existing temp column is in C
...
#this structure is saying pull lake id through (:)daynum, put month there, then pull sample date through : comments. This is allowing you to move the new month column you just created from the very end of the dataset to next to sample date, where it makes more sense to have the month column
...
#two different ways to find first and last dates of the dataset, the second one is a dplyr function
