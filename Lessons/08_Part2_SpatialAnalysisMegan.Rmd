...
#Reveal the extent of this dataset via the st_bbox() function. Anything with st_ is a spatial command assuming you have the sf package loaded. This creates a bounding box around all of the features. THis function gives you the coordinates (spatial extent) of the box around the area of interst.
st_bbox(counties_sf)
...
mapView(counties_sf, zcol = "ALAND") #ALAND is the column with land area, so this command will color each county according to area. AWATER does the same except for water. 
```
...
mapview(huc8_sf, col.regions = 'orange') +
          mapview(SubHuc8_sf, col.regions = 'purple')
...
```{r load.geojson.data}
powerplants_sf <- st_read('https://opendata.arcgis.com/datasets/ee0263bd105d41599be22d46107341c3_0.geojson')#you just paste in the GeoJSON link
...
nc_powerplants_sf %>% 
  st_drop_geometry() %>% 
  count(TYPE, sort=TRUE)  #counts the different types (types, then n column shows how many they're are)
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #this function is ensuring the FIPS gets imported as a factor because it's a numerical category.
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)

#Check structure
str(svi2018_nc_raw)

#Join the SVI attributes to the county spatial features to get geometry 
counties_sf_join <-  merge(x = counties_sf,
                           y = svi2018_nc_raw, 
                           by.x = "GEOID", 
                           by.y = "FIPS" )  #by.x and by.y are the columns that are common among the two datasets.
...
wind_energy <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>% 
        mutate(HUC_8 = substr(HUC_12, 1, 8)) %>% #substr is saying from that string (HUC12) grab 
        group_by(HUC_8) %>%                          #the first through the 8th value
        summarize(kWhkm2day = sum(kWhkm2day))


  
#Join to HUC_8 features (two ways to do this)
HUC_8_windenergy <- huc8_sf %>% 
                  left_join(wind_energy, by = c("HUC_8"="HUC_8"))
...
#View the outputs
mapview(HUC_8_windenergy, zcol = "kWhkm2day")
...
#collapsing all of the counties into this one area
...
#two ways of selecting counties that fall in the Neuse River Basin 

#Select intersecting counties using matrix subsetting
neuse_intersect_1 <- counties_sf_utm[neuse_sf,]
#counties are the dough, the neuse is the cookie cutter (subset counties based on the neuse huc)
...
#View the result (spatial overlay of the intersecting counties and the NRBasin)
myMap + mapview(neuse_intersect_2, alpha.regions = 0)

#Actually intersect the counties (not just select those that intersect); chop off the parts of the counties that are outside of the Neuse River Basin (cookie cutter)--this is actually modifying the data
...
neuse_counties_sf <- neuse_counties_sf %>% 
  mutate(Area_m2 = as.numeric(st_area(neuse_counties_sf$geometry))) #st_area will apply the area to the newly selected geomtetry
...
trianglecounties_sf <- counties_sf_utm %>% 
  filter(NAME %in% c("Chatham","Durham","Orange","Wake"))

  
#Grab the intersecting HUC_8s
HUC_8_intersect <- huc8_sf_utm[trianglecounties_sf,]


#Intersect the HUC_8s
HUC_clip <- huc8_sf_utm %>% 
  st_intersection(trianglecounties_sf)

mapview(HUC_clip)+mapview(trianglecounties_sf, alpha.regions = 0, color = "red")
```
...
#Combine the centroids into one feature and draw voronoi polygons; defines the area that is closest to a given point 
...
#User coordinates you can select a location based on long/lat
userLat = 36.0045442
userLng = -78.9426381
...
result_Mask30km <- st_intersects(counties_sf_utm, the_site_counties, sparse = FALSE)

sel_counties_30km <- counties_sf_utm[result_Mask30km,]
mapview(sel_counties_30km)
...
ggplot()  +
  ...
  scale_fill_gradient(low="yellow", high="darkgreen") #the state boundary at the end (meaning on top) will bury everything else.
...
#here, the state boundary is at the top, or getting drawn first, so it doesn't cover up everything that comes after it.
```
