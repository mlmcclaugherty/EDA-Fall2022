...
-   **points** - created from a single coordinate pair;  ~~the trees~~
-   **lines** - created from a sequence of coordinate pairs representing the start, the end, and any vertex along the line (i.e. where it changes direction); ~~the stream~~ and
-   **polygons** - also created from a sequence of coordinate pairs, but where the first and last pair are in the same location, thus closing the shape and defining an area. ~~the lake~~

~~geometries are how we describe spatial features~~
...
GasPlants <- st_read('https://opendata.arcgis.com/datasets/a0722062315143a7980cebb79a204781_4.geojson') #the sf package comes with the st_read function, which allows you to read data straight from websites. Reads in GeoJSON format.

#Alternative
#GasPlants <- st_read('./Data/Spatial/Manufactured_Gas_Plant_Sites.geojson')

#Explore
class(GasPlants)     #this reveals it's a dataframe and an sf object, meaning you can do spatial analysis on it
names(GasPlants)
str(GasPlants$geometry)  #tells you the spatial feature class of the geometry column 

#View using sf plot function. Status is active or inactive.
plot(GasPlants['status'],pch=16)

#View using ggplot geom sf instead of sf plot
ggplot(data=GasPlants) + geom_sf()

#View using mapview. Shows where they are on the map with the attributes of the points.
mapview(GasPlants)
...
#manipulating data that has location information but not a geometry column and converting it to a spatial dataframe.
...
#this dataframe has latitude and longitude information so that can be used to create the geometry.
...
#Convert to a spatial dataframe by using the st_as_sf function by inputting the x and y coordinate columns. LONGITUDE IS X, LATITUDE IS Y
...
#R can generate the point based on the the long/lat but it has no concept of where that is (no background map) without the coordinate reference system (cfs)

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326)  #linking the geometry to it's lcoation Earth, st_sfc is simple features column (analog to geometry column)
class(sfc_GraingerHall)
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe so it has attributes
...
Each of the above datasets we explored use lat/long coordinates in defining their geometries. We actually specify that these use lat/long coordinate when creating them via the `crs = 4326` parameter you see in the examples. But what is "4326"?? And what is the significance of this? 
 ~~if you wanted to know the distance from Grainger Hall to somewhere else, you could find that but it would be in degrees, which isn't particularly useful. We need to transform from long/lat which are spherical or angular coordinates to planar/cartesian coordinates from which we can calculate linear distances or areas USING PROJECTION~~
Let's explore:
...
#Buffer Grainger Hall 1 unit. Expanding a geometry out by a certain unit of measurement.
the_buffer <- sf_GraingerHall %>% st_buffer(1)

#View the result
mapview(the_buffer)
#it's ovular and goes way up into Virginia...why? Since the data was in degrees (long/lat) the 1 in the st buffer function says buffer by 1 degree lat and long. The buffer is distorted because it's trying to compress the spherical data into a flat/planar surface
...
#Transform the datasets to use UTM 17 N, WGS84. We can show the distance from each gas plant to the epa air monitoring sites. We project the data since it's degrees and we want it in meters to get distance.
EPAair.sf.utm <- st_transform(EPAair.sf,32617)
GasPlants.utm <- st_transform(GasPlants,32617)
#st transform reprojects the crs. The geometry column is now in meters, showing the distance from the center of the coordinate system in eastings and northings 

#Have a look at the coordinate associated with the geometries
EPAair.sf.utm$geometry[1]

#Plot the data to visualize location of gas plants relative to air monitoring sites
ggplot() +
...
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T) #pairwise creates a distrance matrix based on your two input geometries

#Compute the mean of each row (each EPA site)
mean_distances <-  rowMeans(distance_matrix) #reduces dataset to just means
