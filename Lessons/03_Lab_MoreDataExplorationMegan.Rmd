...
vector1[3] #pulls third item from vector 1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # matrix is same data type, multiple dimensions. First value is 1 through 20, nrow is number of rows, ncol is number of columns
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #byrow=true adds values by row (the default is false)
matrix2
...
matrix1[4, ] #leaving column blank, you get all of row 4
matrix1[ , 3] #gives all values in column 3, leaving rows blank
matrix1[c(12, 14)] #12th and 14th item in the matrix, by counting down rows
matrix1[c(12:14)] #colon : indicates slice, so 12 through 14th values
matrix1[2:4, 1:3] #
...
list1[[2]] #list of 4 items, various types
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # allows you to give your own names because R automatically defaults to the vector names

dataframe1[1:2,] # 
dataframe1[c("ID","Passed")] # you can refer to column names to view those columns
dataframe1$ID #this calls the individual column, ID
...
```{r DataExercise-1a}
tendayhighF <- c(93,93,73,78,85,82,78,75,73,71)
tendayhighF
tendaylowF <- c(65,53,48,59,61,54,51,51,52,55)
...
```{r DataExercise-1b}
tendayhighC <-((tendayhighF-32)*5/9)
tendaylowC <- ((tendaylowF-32)*5/9)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
tendaydataframe <-data.frame(tendayhighF,tendaylowF,tendayhighC,tendaylowC)
names(tendaydataframe)<-c("High F", "Low F", "High C", "Low C")
tendaydataframe
```
...
```{r DataExercise-1d}
summary(tendaydataframe)
lapply(tendaydataframe, sd)
```
