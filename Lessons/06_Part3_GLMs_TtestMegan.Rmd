...
```{r, message = FALSE}
EPAair <- read.csv("/home/guest/EDA-Fall2022/Data/Processed/EPAair_O3_PM25_NC1819_Processed.csv", stringsAsFactors = TRUE)
...
length(EPAair$Ozone) #8976 observations; often an indication that the mean is a good representation of the population

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") #this is how you stated your hypothesis, that mu is equal or less than 50
O3.onesample  #p-value is less than 0.05, so reject null hypothesis. The true mean is less than 50 (the alternative hypothesis)
...
# Format as a GLM
O3.onesample2 <- lm(Ozone ~ 1, EPAair)  #using the lm function to find the mean of this series. The ~1 means there's no explanatory variable here, just want the intercept term
O3.onesample2
```
...
ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly()
#plotted out, these look fairly similar. You can use the t-test to get actual values of the difference in ozone levels in 2018 and 2019
...
O3.twosample
#again p-value is less than 0.05 so reject the null. T-test says that the difference in the two means is not equal to zero.

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year)
summary(O3.twosample2)
#reject null, year is predictor of ozone levels.
```
